...
export const presetOptions: { key: PresetKey; label: string; sub: string }[] = [
  {
    key: "foundation",
...
const materiOptions: { key: keyof typeof MN; label: string; sub: string }[] = [
...
export function usePricingBuilder({
  isEditable,
  canViewCostDetails,
  seed,
}: {
  ...
  if (seed && seed.id !== seededId) {
    ...
    setMateri(seed.materi.toLowerCase() as keyof typeof MN);
    setAddons({
      assessment: seed.addon_assessment,
      klinik: seed.addon_klinik,
      klinikSesi: seed.addon_klinik_sesi,
      rekaman: seed.addon_rekaman,
      sertifikat: seed.addon_sertifikat,
      sertifikatQty: seed.addon_sertifikat_qty,
      perjalanan: seed.addon_perjalanan,
      perjalananRp: Number(seed.addon_perjalanan_rp),
    });
            ...
            >
              ...
              <div className="flex rounded-md border border-gray-900/15 bg-white p-0.5 dark:border-white/10 dark:bg-zinc-900">
                <button
                  ...
                  disabled={!isEditable}
                  onClick={() =>
                    updateDay(d.id, { format: "offline" as SessionFormat })
                  }
  ...
  const commercialContent = (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 [&>*]:min-w-0">
      <AppInput
        ...
        value={bdPct}
        onChange={(e) =>
          setBdPct(Math.max(0, Math.min(25, parseFloat(e.target.value) || 0)))
        }
      />
      <AppInput
        inputId="dc-pct"
        ...
        value={dcPct}
        onChange={(e) =>
          setDcPct(Math.max(0, Math.min(30, parseFloat(e.target.value) || 0)))
        }
      />
    </div>
  );
  ...
  const activeAddonLabels = [
    addons.assessment && "Assessment",
    addons.klinik && `Klinik ×${addons.klinikSesi}`,
    addons.rekaman && "Rekaman + LMS",
    addons.sertifikat && `Sertifikat ×${addons.sertifikatQty}`,
    addons.perjalanan && "Perjalanan",
  ...
  return {
    ...
    days,
    totalSesi,
    materiPricePerSesi: MP[materi],
    addonsTotalPrice: addonsBreakdown.price,
    packageContent,
    daysContent,
    materiContent,
    addonsContent,
