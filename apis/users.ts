...
type ApiUserEntry = Omit<
  UserEntry,
  "role" | "job_function" | "data_scope" | "status"
> & {
  role: string;
...
function toUserEntry(entry: ApiUserEntry): UserEntry {
  return {
    ...entry,
...
): Promise<ApiEnvelope<{ list: UserEntry[]; metapaging: MetaPaging }>> {
...
export async function getUserDetails(
  id: string
): Promise<ApiEnvelope<{ user: UserEntry; ownership: UserOwnership }>> {
  const result = await callApi<{
    user: ApiUserEntry;
    ownership: UserOwnership;
  }>("/api/v1/users/details", { token: await getSessionToken(), body: { id } });
  ...
  return {
    ...result,
    data: {
      user: toUserEntry(result.data.user),
...
export async function inviteUser(
...
): Promise<ApiEnvelope<{ user: UserEntry }>> {
  const result = await callApi<{ user: ApiUserEntry }>("/api/v1/users/invite", {
    token: await getSessionToken(),
    body: {
      ...payload,
  ...
  return { ...result, data: { user: toUserEntry(result.data.user) } };
}

export async function updateUser(
  payload: UpdateUserPayload
): Promise<ApiEnvelope> {
  return callApi("/api/v1/users/update", {
    token: await getSessionToken(),
    body: {
      ...payload,
      role: down(payload.role),
