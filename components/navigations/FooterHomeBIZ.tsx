    ...
    <footer className="bg-[linear-gradient(180deg,var(--color-biz-forest)_0%,var(--color-biz-forest-mid)_100%)] pt-13.5 pb-6 text-white">
      <div className="mx-auto grid w-full max-w-315 gap-10 px-4.5 sm:px-7.5 lg:grid-cols-[1fr_1.5fr] lg:gap-16">
        <div>
          ...
          <p className="mt-4"><a href="https://wa.me/6285110545698" target="_blank" rel="noreferrer" onClick={() => trackWhatsAppLead({ placement: "footer" })} className="text-sm font-medium text-biz-lime hover:text-white">Diskusikan Kebutuhan Tim</a></p>
        </div>
        <div className="grid grid-cols-2 gap-7 sm:grid-cols-3">
          {columns.map((column) => (
            <div key={column.title}>
              ...
              {column.links.map(([label, href]) => <a key={label} href={href} onClick={href.includes("wa.me") ? () => trackWhatsAppLead({ placement: "footer_nav" }) : undefined} className="mt-3 block text-[13px] text-white/68 transition-colors hover:text-white">{label}</a>)}
            </div>
          ))}
        </div>
      </div>
