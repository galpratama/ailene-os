...
export default function CurriculumHomeBIZ() {
  ...
  return (
    <section
      id="curriculum"
      className="bg-[linear-gradient(180deg,var(--color-biz-paper),var(--color-biz-paper-end))] py-18 sm:py-28"
    >
      <PageMargin>
        ...
        <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,1.06fr)_minmax(360px,0.94fr)]">
          <div
            ...
            aria-label="Enam modul kurikulum"
            className="overflow-hidden rounded-xl border border-biz-forest-light/15 bg-white/78"
          >
            {modules.map((module, index) => {
              ...
              return (
                ...
                >
                  <span className="grid size-8.5 place-items-center rounded-full bg-biz-lime text-[13px] font-semibold text-biz-forest">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <span>
                    <strong className="block text-[15px] font-medium tracking-[-0.03em] text-biz-ink">
                      {module.title}
                    </strong>
                    <span className="mt-1 block max-w-117.5 text-xs leading-[1.45] text-biz-muted">
                      {module.copy}
                    </span>
                  </span>
                  <ArrowRight
                    size={18}
                    className={`text-biz-forest-light transition-transform ${active ? "translate-x-1" : ""}`}
                  />
                </button>
              );
            })}
          ...
          <div className="relative pb-7">
            <figure className="overflow-hidden rounded-2xl border border-biz-forest-light/15 bg-biz-forest shadow-[0_20px_60px_rgba(6,35,25,0.1)]">
              ...
              <figcaption className="px-4.5 pt-3.5 pb-16 text-xs leading-relaxed text-white/72 lg:pb-4">
                Latihan dirancang untuk bergerak dari pemahaman ke workflow yang
                dipakai.
              </figcaption>
            </figure>
            ...
            <aside
              role="tabpanel"
              className="relative mx-3.5 -mt-12 rounded-2xl border border-biz-forest-light/15 bg-white/96 p-5 shadow-[0_18px_50px_rgba(6,35,25,0.16)] lg:absolute lg:right-0 lg:bottom-0 lg:m-0 lg:w-[min(86%,360px)] lg:p-5.5"
            >
              <span className="block text-[9px] font-semibold tracking-[0.12em] text-biz-forest-light uppercase">
                {String(activeIndex + 1).padStart(2, "0")} ·{" "}
                {activeModule.kicker}
              </span>
              <h3 className="mt-2 text-xl font-medium tracking-[-0.045em] text-biz-forest">
                Tim Anda akan:
              </h3>
              <p className="mt-2 text-xs leading-[1.55] text-biz-muted">
                {activeModule.lead}
              </p>
              <ul className="mt-4.5 grid gap-2.5 border-t border-biz-forest/10 pt-4">
                {activeModule.points.map((point) => (
                  <li
                    key={point}
                    className="grid grid-cols-[16px_1fr] gap-2 text-xs leading-[1.45] text-biz-muted"
                  >
                    <Check size={15} className="mt-0.5 text-biz-forest-light" />
                    {point}
                  </li>
                ))}
              </ul>
              <div className="mt-4.5 border-t border-biz-forest/10 pt-4">
                <span className="text-[11px] font-medium tracking-[0.08em] text-biz-forest-light uppercase">
                  Output utama
                </span>
                <p className="mt-2 text-xs leading-normal text-biz-muted">
                  {activeModule.result}
                </p>
              </div>
            </aside>
