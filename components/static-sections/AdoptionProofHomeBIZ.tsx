...
const popIn: Variants = { hidden: { opacity: 0, scale: 0.4 }, visible: { opacity: 1, scale: 1 } };
const labelIn: Variants = { hidden: { opacity: 0, y: 4 }, visible: { opacity: 1, y: 0 } };

export default function AdoptionProofHomeBIZ() {
  const shouldReduceMotion = useReducedMotion();
  const d = (seconds: number) => (shouldReduceMotion ? 0 : seconds);
      ...
      <div className="mx-auto grid w-full max-w-315 items-center gap-11 px-4.5 sm:px-7.5 lg:grid-cols-[minmax(300px,0.8fr)_minmax(0,1.2fr)] lg:gap-20">
        ...
        <div className="min-w-0 rounded-2xl border border-biz-lime/35 bg-white/3 p-4 sm:p-7.5">
          ...
          <motion.svg
            ...
            role="img"
            aria-labelledby="adoption-chart-title adoption-chart-desc"
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <g fill="none" strokeWidth="1">
              ...
              <motion.path
                ...
                strokeDasharray="4 6"
                strokeWidth="1.5"
                className="stroke-biz-lime/50"
                variants={fadeIn}
                transition={{ duration: 0.4, delay: d(0.65) }}
              />
            </g>
            <motion.path
              d="M72 86 C126 42 190 54 250 112 C318 172 376 211 448 222 C500 230 536 220 560 205"
              fill="none"
              strokeDasharray="5 6"
              strokeLinecap="round"
              ...
              variants={fadeIn}
              transition={{ duration: d(1.3), ease: "easeOut" }}
            />
            <motion.path
              d="M72 86 C126 42 190 54 250 112 C318 70 376 38 448 26 C500 20 536 22 560 30"
              fill="none"
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth="4"
              ...
              transition={{ duration: d(1.3), ease: [0.65, 0, 0.35, 1] }}
            />
            <motion.circle cx="72" cy="86" r="5" className="fill-white" variants={popIn} transition={{ duration: 0.3, delay: d(0.1) }} />
            <motion.circle
              cx="250"
              cy="112"
              r="6"
              className="fill-biz-forest stroke-biz-lime"
              strokeWidth="3"
              variants={popIn}
              transition={{ duration: 0.3, delay: d(0.65) }}
            />
            <motion.circle cx="560" cy="30" r="6" className="fill-biz-lime" variants={popIn} transition={{ duration: 0.3, delay: d(1.3) }} />
            {!shouldReduceMotion && (
              <motion.circle
                cx="560"
                cy="30"
                ...
                whileInView={{ opacity: [0, 0.55, 0], scale: [1, 1, 2.6] }}
                viewport={{ once: true, amount: 0.4 }}
                transition={{ duration: 2.2, repeat: Infinity, ease: "easeOut", delay: 1.3 }}
              />
            )}
            <motion.circle cx="560" cy="205" r="6" className="fill-white/50" variants={popIn} transition={{ duration: 0.3, delay: d(1.3) }} />
            <g className="fill-white/45 text-[10px]">
              ...
              <motion.text x="445" y="198" variants={fadeIn} transition={{ duration: 0.4, delay: d(1.5) }}>momentum turun</motion.text>
            </g>
            <g className="fill-biz-lime text-[10px] font-semibold">
              <motion.text x="202" y="96" variants={labelIn} transition={{ duration: 0.4, delay: d(0.75) }}>follow-through dimulai</motion.text>
              <motion.text x="466" y="20" variants={labelIn} transition={{ duration: 0.4, delay: d(1.5) }}>terus dipakai</motion.text>
            </g>
          </motion.svg>
