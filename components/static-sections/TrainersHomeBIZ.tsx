        ...
        <div className="grid gap-4.5 lg:grid-cols-2">
          {trainers.map((trainer) => (
            <figure key={trainer.name} tabIndex={0} className="group relative h-90 overflow-hidden rounded-2xl bg-biz-forest outline-none focus-visible:ring-3 focus-visible:ring-biz-lime sm:h-95">
              ...
              <img src={trainer.image} alt={`${trainer.name}, ${trainer.role} Ailene`} loading="lazy" decoding="async" className={`h-full w-full object-cover saturate-80 transition-[filter,transform] duration-300 group-hover:scale-[1.02] group-hover:saturate-60 group-hover:brightness-70 group-focus-visible:saturate-60 group-focus-visible:brightness-70 ${trainer.position}`} />
